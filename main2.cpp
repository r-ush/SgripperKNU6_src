...
int32_t 	FakeActualPos[NUM_ELMO_DRIVER ] = {0,};
...
double count_array[2000] = {0,};
long double avg;
long double avg4i = 0;
int initializing_constant = 0;
int fake_real_position = 0;

//Current Control
int16_t target_Current[NUM_ELMO_DRIVER ] = {};

double max_torque = 800;
double max_Position = 3600;
long double sum = 0.0;
long double temp_stand[] = {0,}; 


int32_t selected_actuator_num_current = 0;
int16_t initializing_motor = 0;

long double error[] = {0,0,}; // error값이란 설정값과 출력값의 편차를 말합니다.
long double errorPrevious[] = {0,0,}; //전에 error값을 기억해둡니다.
long double error4I[] = {0,0,}; // error값이란 설정값과 출력값의 편차를 말합니다.
long double errorPrevious4I[] = {0,0,}; //전에 error값을 기억해둡니다.

long double Kp4I[] = {0.2, 7.2, };
long double Ki4I[] = {0, 0.5, };
long double Kd4I[] = {0, 0.6, };
// long double Kp4I[] = {0.1, 7.2, };
// long double Ki4I[] = {0.002, 0.5, };
// long double Kd4I[] = {0.009, 0.6, };
long double PControl4I[] = {0,0, };
long double IControl4I[] = {0,0, };
long double DControl4I[] = {0,0, }; // 비례, 적분, 미분 제어항의 결과 값입니다.
int target_torque[] = {0,0,};	
int target_torque4I[] = {0,0,};	
...
void initializing_function(int initializing_index);
...
class RosNodeClass
{
	public:
		...
		{
			std::cout << "initializing: " << msg.data << " selected\n";
...
void print_run(void *arg)
{
	...
	while (1)
	{
		...
		if (1)
		{
			...
			{
				rt_printf("\n\e[32;1m\tPID value: %d Slave %d / Tor: %d Pos: %d  Vel: %d  \e[0m", ActualTor[i], i, TargetTor[i], ActualPos[i], ActualVel[i]);  //AuxiliaryPositionActualValue[i]
 ...
 ***************************************************************************/
// double initializing(int temp_stand) {
// 	// int temp_stand = ActualTor[NUM_ELMO_DRIVER];// 기준으로 받는 input


// 	for (int i = 0; i < 2000; i++) {

// 		count_array[i] = ActualTor[NUM_ELMO_DRIVER] - temp_stand;

// 	}

// 	// avg = average(count_array, 2000);

// 	// if (avg < 0.5) {
// 	// 	initializing_constant = ActualPos[NUM_ELMO_DRIVER];
// 	// 	break;
// 	// }
// 	// else
// 	// 	continue;
...
// 	return abs(avg);
// }
// double average(long double a[], int size) {
...
void current_control(int actuator_index) {
	...
	long double Ki[] = {0.5, 0.5, };
	long double Kd[] = {0.6, 0.6, };
	long double PControl[] = {0,0, };
	long double IControl[] = {0,0, };
	long double DControl[] = {0,0, }; // 비례, 적분, 미분 제어항의 결과 값입니다.
	...
	if(actuator_index == 0){
		...
		if(target_torque[0] > (int16_t)max_Position && target_torque[1] > (int16_t)max_Position)
		{
			target_torque[0] = (int16_t)max_Position;
			target_torque[1] = (int16_t)max_Position;
		}
		else if(target_torque[0]< (int16_t)-max_Position && target_torque[1]< (int16_t)-max_Position)
		{
			target_torque[0] = (int16_t)-max_Position;
			target_torque[1] = (int16_t)-max_Position;
		}

		// long double target_torque[] = {0,0,};
		// long double target_position[] = {0,0,};

		// target_torque[actuator_index] = target_Current[actuator_index];
		target_Current[0] = target_Current[actuator_index];
		target_Current[1] = target_Current[actuator_index];

		
		error[0] =  target_Current[0] - ActualPos[0];
		...
		// rt_printf("\n\e[32;1m\t ID: %d Error: %d ActualPos: %d, target_torque %Lf \e[0m", 0, error[0], ActualPos[0], target_torque[0]);
		// rt_printf("\n\e[32;1m\t ID: %d PControl %Lf / IControl: %Lf,  DControl: %Lf" , 0, PControl[0], IControl[0], DControl[0]);

		TargetTor[0] = target_torque[0];

		error[1] =  target_Current[1] - ActualPos[1];
		...
		// rt_printf("\n\e[32;1m\t ID: %d Error: %d ActualPos: %d, target_torque %Lf \e[0m", 0, error[1], ActualPos[1], target_torque[1]);
		// rt_printf("\n\e[32;1m\t ID: %d PControl %Lf / IControl: %Lf,  DControl: %Lf" , 0, PControl[1], IControl[1], DControl[1]);

		TargetTor[1] = target_torque[1];

	// 	// for (int i=0; i< actuator_index; i++){
	// 	// 	error[i] =  target_Current[i] - ActualPos[i];
	// 	// 	PControl[i] = Kp[i] * error[i]; //비례항 입니다.
	// 	// 	IControl[i] += Ki[i] * error[i] * 0.001; // 적분항 입니다.
	// 	// 	DControl[i] = Kd[i] * (error[i] - errorPrevious[i]) / 0.001; // 미분항입니다.
	// 	// 	target_torque[i] = int(PControl[i] + IControl[i] + DControl[i]); // P, I, D 결과값을 더해줍니다.
	// 	// 	errorPrevious[i] = error[i]; //이전 에러값를 기억합니다.
	// 	// 	rt_printf("\n\e[32;1m\t ID: %d Error: %d ActualPos: %d, target_torque %Lf \e[0m", i, error[i], ActualPos[i], target_torque[i]);
	// 	// 	rt_printf("\n\e[32;1m\t ID: %d PControl %Lf / IControl: %Lf,  DControl: %Lf" , i, PControl[i], IControl[i], DControl[i]);
	// 	// TargetTor[i] = target_torque[i];
		// }
	...
	long double Ki[] = {0.5, 0.5, };
	long double Kd[] = {0.6, 0.6, };
		if(target_torque[actuator_index] > (int16_t)max_Position)
		{
			target_torque[actuator_index] = (int16_t)max_Position;
		}
		else if(target_torque[actuator_index]< (int16_t)-max_Position)
		{
			target_torque[actuator_index] = (int16_t)-max_Position;
		}

		error[actuator_index] =  target_Current[actuator_index] - ActualPos[actuator_index];
...
// void initializing_function(int initializing_index){
// 	if (initializing_index == 7){
// 	temp_stand[0] = ActualTor[0];// 기준으로 받는 input
// 		rt_printf("\n\e[32;1m\t temp_stand: %d ActualTor: %d \e[0m", temp_stand[0], ActualTor[0]);
// 		while(1){
// 			for (int count = 0; count < 2000; count++){
// 				error4I[0] =  -1500 - ActualVel[0];
...
// 				count_array[count] = ActualTor[0] - temp_stand[0];
...
// 			double avg4i = sum / 2000.0;
// 			rt_printf("\n\e[32;1m\t Error: %d ACtualVel: %d ActualTor: %d ActualPos: %d avg4i %d sum %d\e[0m", error4I[0], ActualVel[0] , ActualTor[0], ActualPos[0], avg4i, sum);

// 			if (avg < 50.0) {
// 					initializing_constant = ActualPos[0];
// 					rt_printf("\n\e[32;1m\t ID: %d avg: %lf \e[0m", 0, avg); 
// 					rt_printf("\n\e[32;1m\t ID: %d initializing_constant: %d \e[0m", 0, initializing_constant); 
// 					break;
// 			}
// 			else{ 
// 				// rt_printf("\n\e[32;1m\t ID: %d index4I: %d \e[0m", 0, avg); 
// 				continue;
// 			}
// 		}
// 	}
